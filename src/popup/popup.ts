import { MessageType, RecordingStatus, RecordingSession } from '../types';
...
class PopupController {
  ...
  constructor() {
    ...
    this.playbackToggle = document.getElementById('playbackToggle') as HTMLInputElement;
    this.viewRecordingsButton = document.getElementById('viewRecordings') as HTMLButtonElement;
    ...
    this.recordingInfo = document.getElementById('recordingInfo') as HTMLElement;
    this.recordingsSection = document.getElementById('recordingsSection') as HTMLElement;
    this.recordingsList = document.getElementById('recordingsList') as HTMLElement;
  ...
  private async init() {
    ...
    this.viewRecordingsButton.addEventListener('click', () => this.toggleRecordingsList());
  ...
  private renderRecordings() {
    if (this.recordings.length === 0) {
      this.recordingsList.innerHTML = '<div class="empty-state">No recordings yet</div>';
      return;
    }
  ...
  private async deleteRecording(recordingId: string) {
    ...
    try {
      ...
      const filtered = recordings.filter((r: RecordingSession) => r.id !== recordingId);
