...
import { MessageType, RecordingSession } from '../types';
...
class DOMRecorder {
  ...
  private async playRecording(recording: RecordingSession) {
    console.log('[Zordon Recorder] Playing recording:', recording.id, 'with', recording.events?.length || 0, 'events');
    ...
    if (existingScript) {
      console.log('[Zordon Recorder] Player already loaded, sending message directly');
      window.postMessage({
        type: 'ZORDON_PLAY',
        recording: recording
      }, '*');
      ...
      script.onload = () => {
        console.log('[Zordon Recorder] Player script loaded, sending recording');
